# AGENTS.md

This repository is the source of truth for Cassowary World canon. Other applications may render or consume it, but canon is defined here.

Agents must follow the detailed operating rules in [Agent Instructions](docs/agent-instructions.md).

Before writing or revising canon-bearing material, check:

- [Guiding Principles](GUIDING_PRINCIPLES.md)
- [Core Logic](CORE_LOGIC.md)
- [Canon Index](CANON_INDEX.md)
- [Open Questions](99-open-questions.md)
- [Repository Structure](docs/repository-structure.md)

In short: do not invent arbitrary lore, do not silently promote story assumptions to canon, and explain every capability through biology, ecology, technology, or culture.
